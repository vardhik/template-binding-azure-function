...
namespace TemplateGenerate
{
    public class RazorViewToStringRenderer
    {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public async Task<string> RenderViewToStringAsync<TModel>(TModel model, string viewName, string viewLocationPath = null)
        //public async Task<string> RenderViewToStringAsync<TModel>(TModel model, string viewName)
        {
            var actionContext = GetActionContext();
            var view = FindView(actionContext, viewName, viewLocationPath);
        ...
        private IView FindView(ActionContext actionContext, string viewName, string viewLocationPath)
        {
            //viewEngine.GetView("~/Views/Voucher/", "Voucher_TOD.cshtml", isMainPage: false)
            //var getLocationViewResult = viewEngine.GetView(executingFilePath: null, viewPath: viewName, isMainPage: false);
            //if (getLocationViewResult.Success) return getLocationViewResult.View;
            if (!string.IsNullOrWhiteSpace(viewLocationPath))
                viewLocationPath = $"~/Views{viewLocationPath}";
            var getViewResult = viewEngine.GetView(viewLocationPath, viewPath: viewName, isMainPage: false);
